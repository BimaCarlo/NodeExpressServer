...
router.get("/", function (req, res) {//richiamo il mio form
  ...
  if (jwtoken != undefined && jwtoken != null) {//login automatico con local storage
    ...
    let tokenCookie = tokenGenerator.verificaWebToken(req.cookies['jwebtoken'], 'secret');//verrà generato un errore se il token non esiste ma viene gestito dal metodo e scritto in console 
...
router.post("/", function (req, res, next) {//la chiamata in post in questo caso funziona come un middleware
  ...
  tokenGenerator.generaToken(userid, pwd, res).then(
    function (token) {
      let verifiedToken;
      if (token != null) {
        verifiedToken = tokenGenerator.verificaWebToken(token, 'secret');
